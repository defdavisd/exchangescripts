...
Generates a report for all mailboxes on the specified organization.
...
if($organization){ 
    Write-Host "Organization to process:" $organization
    $mailboxes = @(Get-Mailbox -organization $organization -resultsize unlimited -IgnoreDefaultScope) 
}
...
{
    if ($mb.RecipientTypeDetails -ne 'DiscoveryMailbox' -and $mb.DisplayName -ne 'administrator')
    {
	    $i = $i + 1
        ...
        $userObj | Add-Member NoteProperty -Name "FirstName" -Value $user.Firstname
	    $userObj | Add-Member NoteProperty -Name "LastName" -Value $user.LastName
